...
# check
cmd="$RCMD cargo c -F all"; echo "std, safe\n$ " $cmd; $cmd
cmd="$RCMD cargo cu -F all"; echo "std, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cn -F all"; echo "no_std, safe\n$" $cmd; $cmd

# check additional targets
cmd="$RCMD cargo cuT1 -F all"; echo "std, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cuT2 -F all"; echo "std, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cuT3 -F all"; echo "std, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cnuT5 -F all"; echo "no_std, no_alloc, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cuT6 -F all"; echo "std, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cnuT7 -F all"; echo "no_std, no_alloc, unsafe\n$" $cmd; $cmd
cmd="$RCMD cargo cnuT8 -F all"; echo "no_std, no_alloc, unsafe\n$" $cmd; $cmd
